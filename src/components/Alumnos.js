...
export default class Alumnos extends Component {
  ...
  generarSorteo = () => {
    //var count = this.state.alumnos.length;
    var count = 24;
    console.log(count)
    if (count %8 == 0) {
      var numAlumnos = parseInt( count / 8);
      var aux = [];
    console.log(numAlumnos)
      //var auxCaja = [];

      for (var i = 1; i <= numAlumnos; i++) {
        var idrandom = parseInt(Math.floor(Math.random() * this.state.alumnos.length));
        aux.push(idrandom);
      }
      console.log(aux)
      var idCaja = Math.floor(Math.random() * this.state.grupos.length);
      //auxCaja.push(idCaja);
        ...
        <main className='m-4 bg-neutral-900 p-2'>
          ...
          <section className='flex flex-row justify-center gap-2'>
            <div>
              ...
              <button onClick={this.generarSorteo} class="relative  mt-8 border hover:border-sky-600 duration-500 group cursor-pointer text-sky-50  overflow-hidden h-14 w-full rounded-md bg-sky-800 p-2 flex justify-center items-center font-extrabold">
            ...
            <div className='bg-sky-600 rounded-lg p-8 py-16 max-w-7xl w-full flex flex-row justify-center flex-wrap gap-8'>
              ...
              {
                this.state.grupos.map((index => {
                  return (
                    <div className='flex flex-col' key={index}>
                      <span className=' text-xl text-neutral-50 font-bold mb-6'>Equipo {index}</span>
                      <div
